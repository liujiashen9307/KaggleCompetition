...
print(time.ctime())
model = lgb.train(lgb_params, dtrain, 2600)
print('Start predicting')
print(time.ctime())
pred = model.predict(test[col],num_iteration=2600)
print('Prediction Done')
print(time.ctime())
test['reordered'] = pred
sub = sub_file(test,0.20,sample_submission)
sub.to_csv('20170806_Azure_91Feats_0.01_80Leaves_lgb_2600Rounds.csv',index=False)
print('sub file generated')
test_set2 = test[['user_id','product_id','order_id','reordered']]
test_set2.to_csv('save_test_for_exp_azure_91Feats_0.01LR_2600Rounds.csv',index=False)
FI = get_feature_importance_lgb(model)
FI.to_csv('Feature_Importance_91Feats_0.01_80Leaves_lgb_2600Rounds.csv',index=False)

###0806 two subs are both generated here!
...
test['reordered'] = pred
sub = sub_file(test,0.20,sample_submission)
sub.to_csv('20170806_Azure_91Feats_0.03_80Leaves_lgb_700Rounds.csv',index=False)
print('sub file generated')
test_set2 = test[['user_id','product_id','order_id','reordered']]
test_set2.to_csv('save_test_for_exp_azure_91Feats_0.03LR_700Rounds.csv',index=False)
FI = get_feature_importance_lgb(model)
FI.to_csv('Feature_Importance_91Feats_0.03_80Leaves_lgb_700Rounds.csv',index=False)
