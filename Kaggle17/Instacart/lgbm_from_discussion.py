...
order_prior = pd.read_csv(os.path.join(path, "order_products__prior.csv"), dtype={'order_id': np.int32,
                                                                                  'product_id': np.uint16,
                                                                                   'add_to_cart_order': np.int16,
                                                                                   'reordered': np.int8})
order_train = pd.read_csv(os.path.join(path, "order_products__train.csv"), dtype={
                                                                           'order_id': np.int32,
                                                                           'product_id': np.uint16,
                                                                            'add_to_cart_order': np.int16,
                                                                             'reordered': np.int8})
orders = pd.read_csv(os.path.join(path, "orders.csv"), dtype={
                                'order_id': np.int32,
                                'user_id': np.int64,
                                'eval_set': 'category',
...
products = pd.read_csv(os.path.join(path, "products.csv"), dtype={
...
embedings = [str(each) for each in embedings]
...
labels = order_train[['reordered']]

    #data_val = order_test[features]


lgb_train = lgb.Dataset(data, labels)
del data,labels,order_train,order_prior,order_stat,orders,orders_products
del aisles,embedings,departments,features,prod_stat,prod_usr,prod_usr_reordered,product_embeddings
del products,product_periods,user_aisle_stat,user_dep_stat,user_order_stat,user_stat
...
prediction = gbm.predict(order_test[features])
    # prediction = model.predict(data_val)
orders = order_test.order_id.values
products = order_test.product_id.values

result = pd.DataFrame({'product_id': products, 'order_id': orders, 'reordered': prediction})
result.to_csv('save_for_test_20170809_lgbm_model_with_new_feature.csv',index=False)
sub = sub_file(result,0.20,sample_submission)
sub.to_csv('20170809_lgbm_model_with_new_feature.csv',index=False)
