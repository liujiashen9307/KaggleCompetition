...
print(time.ctime())
model = lgb.train(lgb_params, dtrain, 550)
print('Start predicting')
print(time.ctime())
pred = model.predict(test[col],num_iteration=550)
print('Prediction Done')
print(time.ctime())
test['reordered'] = pred
sub = sub_file(test,0.20,sample_submission)
test_set2 = test[['user_id','product_id','order_id','reordered']]
test_set2.to_csv('save_test_for_exp_azure_86Feats_0.1LR_550Rounds.csv',index=False)
sub.to_csv('20170805_Azure_86Feats_0.1_1000Leaves_lgb_550Rounds.csv',index=False)
FI = get_feature_importance_lgb(model)
FI.to_csv('Feature_Importance_1000Leaves.csv',index=False)
