# -*- coding: utf-8 -*-
"""
Created on Tue Aug  1 23:34:43 2017

@author: Fraon, from Kaggle

@purpose: optimize the number of products
"""
...
def choose_threshold(P, pNone=None):
    P = np.sort(P)[::-1]
    n = P.shape[0]
    if pNone is None:
        pNone = (1.0 - P).prod()
    ...
    if opt[0]!=0:
        ...
        threshold = -999
    none_sig = 'Y' if opt[1] else 'N'
    return (threshold,none_sig)
...
sub1 = pd.read_csv('save_test_for_exp_3.csv')
sub2 = pd.read_csv('save_test_for_exp_4.csv')
sub3 = pd.read_csv('save_test_for_exp_5.csv')


Average = 0.75*sub1.reordered + 0.25*sub2.reordered
Average2 = 0.45*Average + 0.55*sub3.reordered

sample = pd.read_csv('data/sample_submission.csv')
sub4 = pd.read_csv('save_test_for_exp_6.csv')
Average3 = 0.5*Average2+0.5*sub4.reordered
sub = sub1
sub['reordered'] = Average3

unique_order = list(set(sub['order_id']))
def get_threshold_order(order_id,df):
    prob = list(df[df['order_id']==order_id]['reordered'])
    return choose_threshold(prob)
#threshold = [get_threshold_order(each,sub)[0] for each in unique_order]
threshold = []
None_l = []
for i in range(len(unique_order)):
    thr,non = get_threshold_order(unique_order[i],sub)
    threshold.append(thr)
    None_l.append(non)
    if i%500==0:
        print(i)
        
threshold_df = pd.DataFrame({'order_id':unique_order,'threshold':threshold,'HasNone':None_l})

sub = sub.merge(threshold_df,on='order_id',how='left')

d = dict()
for row in sub.itertuples():
    if row.threshold==-999:
        d[row.order_id] = 'None'
    elif row.reordered > row.threshold:
        try:
        ...
        if row.HasNone=='Y':
            d[row.order_id] += 'None'
for order in sample.order_id:
    if order not in d:
        d[order] = 'None'
subs = pd.DataFrame.from_dict(d, orient='index')
subs.reset_index(inplace=True)
subs.columns = ['order_id', 'products']

subs.to_csv('submission/20170801_current_best_with_max_FScore_test.csv',index=False)