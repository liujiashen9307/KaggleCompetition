...
FI = pd.read_csv('lgb_FI.csv')
drop_feature = list(FI[FI['Score']<=3334]['Feature'])

"""
去掉15个特征，在之前特征组合的时候就去掉。同时再设法再添加一些特征。
"""
...
del prd['department']
del prd['aisle']
prd_drop = [each for each in drop_feature if '_prod' in each]
prd = prd.drop(prd_drop,axis=1)
...
data = data.merge(priors_orders_detail[['order_id','order_number', 'order_dow',
       'order_hour_of_day','_user_buy_product_times']],on='order_id',how='left')
...
del ostreak
del train, prd, users
del aisles,departments,priors,W2C,products,us
...
col.remove('aisle_id')
col = [each for each in col if each not in drop_feature]
...
model = lgb.train(lgb_params, dtrain, 1000,verbose_eval=4,early_stopping_rounds = 50,valid_sets=[dtrain, validdata])
