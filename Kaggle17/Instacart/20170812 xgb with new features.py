...
print(time.ctime())
model = xgb.train(xgb_params,dtrain,num_boost_round=700,verbose_eval=10, evals=watchlist)
print(time.ctime())
print('Start predicting')
pred = model.predict(dtest)
print('Prediction Done')
print(time.ctime())
pred = list(pd.read_csv('save_test.csv')['value'])
test['reordered'] = pred
sub = sub_file(test,0.20,sample_submission)
test_set2 = test[['user_id','product_id','order_id','reordered']]
test_set2.to_csv('save_test_for_exp_20170812_Azure_86_feats_gap_newfeat_xgb_700_rounds_0.05_depth7.csv',index=False)
sub.to_csv('20170812_Azure_86_feats_gap_newfeat_xgb_700_rounds_0.05_depth7.csv',index=False)
