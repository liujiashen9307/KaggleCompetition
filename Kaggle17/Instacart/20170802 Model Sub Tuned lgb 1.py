...
dtrain = lgb.Dataset(train_set[col],train_set['reordered'])
del train_set
lgb_params = {
    'learning_rate': 0.01,
    'boosting_type': 'gbdt',
...
model = lgb.train(lgb_params, dtrain, 2000)
print('Start predicting')
pred = model.predict(test_set[col])
test_set['reordered'] = pred
sub = sub_file(test_set,0.20,sample_submission)
sub.to_csv('submission/20170802_sub_74_features_full_data_240leaves_0.01_lgb_2000Rounds_0.20thres.csv',index=False)
test_set2 = test_set[['user_id','product_id','order_id','reordered']]
test_set2.to_csv('save_test_for_exp_10.csv',index=False)

FI = get_feature_importance_lgb(model)
FI.to_csv('lgb_FI.csv',index=False)
